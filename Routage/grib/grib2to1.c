...
**    http://rda.ucar.edu/libraries/grib/c_routines/unpackgrib2.c
** It must be in the same directory as this program.
**
** If you want to decode jpeg-compressed grids, you will also need to link with
** the JasPer library, which needs the JPEG-6b library.
...
ParameterData mapParameterData(GRIBMessage *msg,int grid_number)
{
  ParameterData pdata;
  switch (msg->disc) {
...
// Sections, which are both fixed in length
	...
	for (m=0; m < num_points; ++m) {
	  if (grib_msg.grids[n].gridpoints[m] != GRIB_MISSING_VALUE) {
	    ...
	    pvals[cnt]=lround((grib_msg.grids[n].gridpoints[m]-grib_msg.grids[n].md.R)*pow(10.,grib_msg.grids[n].md.D)/pow(2.,grib_msg.grids[n].md.E));
