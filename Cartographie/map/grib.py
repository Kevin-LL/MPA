...
class Grib:
    ...
    def activate(self):
        ...
        self.app.save_button.grid(row=0, column=3, padx=(0, 10), ipadx=5, ipady=5)
        self.app.save_as_button.grid(row=0, column=4, padx=(0, 10), ipadx=5, ipady=5)
        self.app.move_button.grid(row=0, column=5, padx=(0, 10), ipadx=5, ipady=5)
        self.app.edit_button.grid(row=0, column=6, padx=(0, 10), ipadx=5, ipady=5)
        self.app.undo_button.grid(row=0, column=7, padx=(0, 10), ipadx=5, ipady=5)
        self.app.redo_button.grid(row=0, column=8, padx=(0, 10), ipadx=5, ipady=5)
        self.app.history_button.grid(row=0, column=9, ipadx=5, ipady=5)
    ...
    def open(self, file_path = None):
        ...
        if file_path and self.close() is not None:
            ...
            #conversion en .isoc et sauvegarde pour la partie routage
            file_name = os.path.basename(file_path)
            
            remove_dir = Path(f"{Path(__file__).parent.absolute()}/routage/meteo")
            files_grib = os.listdir(remove_dir)
            for file in files_grib:
                os.remove(Path(f"{Path(__file__).parent.absolute()}/routage/meteo/"+file))
            os.rmdir(remove_dir)
            os.mkdir(remove_dir)

            save_path = Path(f"{Path(__file__).parent.absolute()}/routage/meteo/"+file_name)
            shutil.copy(file_path, save_path)
            
            cmd = "./../../Routage/grib/grib2isoc2 "+str(save_path)
            subprocess.call(cmd, shell=True)
            
            actual_grib_path = Path(f"{Path(__file__).parent.absolute()}/out.isoc")
            new_grib_path = Path(f"{Path(__file__).parent.absolute()}/routage/meteo/")
            shutil.move(actual_grib_path, new_grib_path)
                    ...
                    if self.thread_run.is_set():
                        ...
                        if not np.isnan(u) and not np.isnan(v):
                            ...
                            if barb_bbox not in self.barbs_ref and tools.is_intersection(visible_bbox, barb_bbox):
                                ...
                                speed_in_knot = speed_in_ms / 0.514444
                                speed = 5 * round(speed_in_knot / 5) # Arrondi à un multiple de 5
                                if speed > 150:
                                    speed = 150
            ...
            for index_longitude, longitude in enumerate(self.data_longitudes):
                ...
                if latitude >= min_latitude and latitude <= max_latitude and longitude >= min_longitude and longitude <= max_longitude:
                    ...
                    if type == "VAL_SPEED":
                        if is_offset:
                            speed_in_ms = tools.wind_uv_to_speed(u, v)
                            new_speed = speed_in_ms + float(input_value)
                            if new_speed <= 0:
                                new_speed = 1
